...
# present and correctly differentiates OOS vs in-stock on mshop.vivo.com, but
...
# fallback — a Vivo OOS page still shows the price, so treating "price present"
...
def _log_diagnostics(soup: BeautifulSoup, html: str) -> None:
    ...
    logger.info(f"[vivo][diag] HTML length={len(html)}, head={html[:200]!r}")
    ...
    for script in soup.find_all("script", type="application/ld+json"):
        ...
        for item in (data if isinstance(data, list) else [data]):
            if isinstance(item, dict) and item.get("offers") is not None:
                ...
                if avail:
                    found_ld = True
                    logger.info(f"[vivo][diag] JSON-LD availability={avail!r}")
    if not found_ld:
        logger.info("[vivo][diag] JSON-LD availability: none found")
    ...
    for key in (
        ...
        if key in html:
            logger.info(f"[vivo][diag] embedded JSON key present: {key!r}")

    oos_hits = [p for p in _OOS_PATTERNS if p in html_lower]
    logger.info(f"[vivo][diag] OOS text patterns present: {oos_hits or 'none'}")
...
def check(soup: BeautifulSoup, html: str) -> bool:
        ...
        try:
            ...
            for item in (data if isinstance(data, list) else [data]):
                ...
                if not avail:
                    continue
                if "InStock" in avail:
                    logger.info("[vivo] JSON-LD: InStock → True")
                    return True
                if "OutOfStock" in avail or "Discontinued" in avail:
                    logger.info("[vivo] JSON-LD: OutOfStock/Discontinued → False")
                    return False
        except Exception:
            pass
    ...
    for key in ('"in_stock":true', '"inStock":true', '"is_available":true', '"isAvailable":true'):
        if key in html:
            logger.info(f"[vivo] embedded JSON {key!r} → True")
            return True
    for key in ('"in_stock":false', '"inStock":false', '"is_available":false', '"isAvailable":false'):
        if key in html:
            logger.info(f"[vivo] embedded JSON {key!r} → False")
            return False

    # ── Explicit OOS text (last-resort negative signal) ───────────────────────
    for pattern in _OOS_PATTERNS:
        if pattern in html_lower:
            logger.info(f"[vivo] OOS text: '{pattern}' → False")
            return False

    logger.info("[vivo] no conclusive signal → defaulting OUT OF STOCK (False)")
    return False
